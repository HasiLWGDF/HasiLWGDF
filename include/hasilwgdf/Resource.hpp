...
namespace Hasibix::HasiLWGDF::Core
{
	...
	private:
		...
		Resource(const std::string &identifier, const ResourceType &type,
				 const std::string &path, std::unique_ptr<unsigned char[]> pData) : identifier(identifier), path(path), type(type), pData(pData)
		{
		}

	public:
		static Holder<Resource> create(const std::string &identifier,
									   ...
									   const unsigned char pData[])
		{
			...
			Holder<Resource> pResource = {Holder<Resource>{Resource{identifier,
																	type, path, std::move(data)}}};
