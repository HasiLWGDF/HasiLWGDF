...
namespace Hasibix::HasiLWGDF::Core
{
	...
	private:
		...
		void preUpdate(float delta)
		{
			for (Holder<Module> module : modules)
			{
				module.get()->getOnPreUpdate().run(delta);
			}
			for (Holder<Item> child : children)
			{
				child.get()->preUpdate(delta);
			}
		}
		...
		void postUpdate(float delta)
		{
			for (Holder<Module> module : modules)
			{
				module.get()->getOnPostUpdate().run(delta);
			}
			for (Holder<Item> child : children)
			{
				child.get()->postUpdate(delta);
			}
		}
	...
	public:
		Item(Holder<Math::Transform> &transform, std::string name = "New Item") : transform(transform), name(name)
		{
		}
