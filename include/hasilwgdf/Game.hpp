...
extern "C"
{
	namespace Hasibix::HasiLWGDF::Core
	{
		...
		private:
			...
			static void preUpdate(float delta)
			{
				groups.at(currentGroup).get()->preUpdate(delta);
			}
			...
			static void postUpdate(float delta)
			{
				groups.at(currentGroup).get()->postUpdate(delta);
			}
