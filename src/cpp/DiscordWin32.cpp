...
public:
	DiscordWin32(const std::string &clientId) :
			_Discord_(), pipeHandle(INVALID_HANDLE_VALUE), clientId(clientId) {
	}
...
bool DiscordWin32::connect() {
	pipeHandle = CreateFileA(PIPE_NAME.c_str(), GENERIC_WRITE, 0, nullptr,
...
bool DiscordWin32::sendData(const std::string &data) {
	DWORD bytesWritten;
	bool success = WriteFile(pipeHandle, data.c_str(),
			static_cast<DWORD>(data.size()), &bytesWritten, nullptr) != 0;
	return success;
}
