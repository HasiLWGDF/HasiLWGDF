...
{
    const std::unique_ptr<Logger> pLogger = std::make_unique<Logger>(Logger("Game"));
    const std::unique_ptr<Logger> pDLogger = std::make_unique<Logger>(Logger("Discord"));
    ...
    int _Game_::loop(SDL_Event &event, std::unique_lock<std::mutex> &lock, void (*initialize)(), void (*earlyUpdate)(float), void (*update)(float), void (*lateUpdate)(float), void (*destroy)())
    {
        double delta;
    ...
    int _Game_::discord(std::unique_ptr<Discord::Activity> &activity, std::unique_lock<std::mutex> &lock)
    {
        ...
        if (!discordRPC->init())
        {
            pDLogger->log(Logger::Level::ERR, "Unable to initialize Discord RPC/IPC.");
            return -1;
        }
    ...
    int _Game_::run(std::unique_ptr<Discord::Activity> &activity, void (*initialize)(), void (*earlyUpdate)(float), void (*update)(float), void (*lateUpdate)(float), void (*destroy)())
    {
        ...
        std::thread loopThread([&]()
                               {
            std::unique_lock<std::mutex> lock(gameRunningMutex);
            loop(event, lock, initialize, earlyUpdate, update, lateUpdate, destroy); });
    ...
    extern "C" HASILWGDF_EXPORT int Game::run()
    {
        ...
        void (*earlyUpdate)(float) = [](float delta){
            Game::earlyUpdate(delta);
        };
        void (*update)(float) = [](float delta){
            Game::update(delta);
        };
        void (*lateUpdate)(float) = [](float delta){
        ...
        int exitCode = _Game_::instance()->run(Discord::currentActivity,
                                               initialize,
                                               earlyUpdate,
                                               update,
                                               lateUpdate,
