...
{
    std::unique_ptr<Logger> pLogger = std::make_unique<Logger>(Logger("Input.Joystick"));
    ...
    void _Joystick_::onJoystickConnected(SDL_ControllerDeviceEvent &e)
    {
        ...
        if (SDL_IsGameController(deviceIndex))
        {
            ...
            if (c->gc)
            {
                ...
                pLogger->log(Logger::Level::DEBUG, "Joystick connected: mapIndex(%s), deviceIndex(%s)", mapIndex, deviceIndex);
                availableJoysticks[mapIndex] = std::move(c);
            }
            else
            {
                pLogger->log(Logger::Level::ERR, "Error opening game controller with Device Index %s: %s", deviceIndex, SDL_GetError());
            }
        }
    }
    ...
    void _Joystick_::onJoystickDisconnected(SDL_ControllerDeviceEvent &e)
    {
        ...
        for (auto it = availableJoysticks.begin(); it != availableJoysticks.end(); it++)
        {
            ...
            {
                pLogger->log(Logger::Level::DEBUG, "Joystick disconnected: %s", deviceIndex);
    ...
    bool _Joystick_::getJoystickDown(int joystick, Joystick::JoystickBtn btn)
    {
        ...
        pLogger->log(Logger::Level::ERR, "Joystick with id %s is not available!", joystick);
        return false;
    }
    ...
    void _Joystick_::update()
    {
        for (auto it = availableJoysticks.begin(); it != availableJoysticks.end(); it++)
        {
            ...
            if (!(c && c->gc))
            {
                pLogger->log(Logger::Level::DEBUG, "Invalid game controller in availableJoysticks map.");
            }
            if (c && c->gc)
            {
                ...
                for (unsigned int i = 0; i < static_cast<int>(Joystick::JoystickBtn::H_JB_COUNT); i++)
                {
                    c->buttons[i] = SDL_GameControllerGetButton(c->gc, static_cast<SDL_GameControllerButton>(i));
                }

                for (unsigned int i = 0; i < static_cast<int>(Joystick::JoystickAxis::H_JA_COUNT); i++)
                {
    ...
    extern "C" HASILWGDF_EXPORT bool Joystick::getJoystickDown(int joystick, JoystickBtn btn)
    {
        return _Joystick_::instance()->getJoystickDown(joystick, btn);
    }

    extern "C" HASILWGDF_EXPORT bool Joystick::getJoystickPressed(int joystick, JoystickBtn btn)
    {
        return _Joystick_::instance()->getJoystickPressed(joystick, btn);
    }

    extern "C" HASILWGDF_EXPORT bool Joystick::getJoystickReleased(int joystick, JoystickBtn btn)
    {
        return _Joystick_::instance()->getJoystickReleased(joystick, btn);
    }

    extern "C" HASILWGDF_EXPORT float Joystick::getJoystickAxis(int joystick, JoystickAxis axis)
    {
