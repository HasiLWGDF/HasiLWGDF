...
namespace Hasibix::HasiLWGDF::Core::Input
{
	...
	bool _Touch_::getTouchDown(int device, int finger, int posX, int posY)
	{
		...
		{
			if (pTouchState[i].device == device && pTouchState[i].finger == finger && pTouchState[i].posX == posX && pTouchState[i].posY == posY && pTouchState[i].isTouchDown)
			{
				return true;
			}
		}
	...
	bool _Touch_::getTouchPressed(int device, int finger, int posX, int posY)
	{
		for (int i = 0; i < numTouchStates; i++)
		{
			if (pTouchState[i].device == device && pTouchState[i].finger == finger && pTouchState[i].posX == posX && pTouchState[i].posY == posY && !pPrevTouchState[i].isTouchDown && pTouchState[i].isTouchDown)
			{
				return true;
			}
		}
	...
	void _Touch_::update(const SDL_Event &event)
	{
		if (event.type == SDL_FINGERDOWN)
		{
			...
			memcpy(pPrevTouchState, pTouchState,
				   (numTouchStates - 1) * sizeof(TouchState));
	...
	extern "C" HASILWGDF_EXPORT bool Touch::getTouchDown(int device, int finger,
														 int posX, int posY)
	{
		return _Touch_::instance()->getTouchDown(device, finger, posX, posY);
	}

	extern "C" HASILWGDF_EXPORT bool Touch::getTouchPressed(int device, int finger,
															int posX, int posY)
	{
		return _Touch_::instance()->getTouchPressed(device, finger, posX, posY);
	}

	extern "C" HASILWGDF_EXPORT bool Touch::getTouchReleased(int device, int finger,
															 int posX, int posY)
	{
